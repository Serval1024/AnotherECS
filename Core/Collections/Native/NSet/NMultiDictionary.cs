...
{    
        ...
        where THashProvider : struct, IHash<TKey, uint>
    {
        ...
        public NMultiDictionary(TAllocator* allocator, uint capacity)
        {
            _count = 0;
            _freeCount = 0;
            _freeList = -1;
        ...
        {
            private readonly NMultiDictionary<TAllocator, TKey, TValue, THashProvider> _data;
            private readonly uint _key;

            public ValuesByKeyEnumerable(ref NMultiDictionary<TAllocator, TKey, TValue, THashProvider> data, uint key)
            {
                _data = data;
                _key = key;
            }
            ...
            public struct ValuesByKeyEnumerator : IEnumerator<TValue>, IEnumerator
            {
                ...
                [MethodImpl(MethodImplOptions.AggressiveInlining)]
                internal ValuesByKeyEnumerator(in NMultiDictionary<TAllocator, TKey, TValue, THashProvider> data, uint key)
                {
                    _data = data;
