...
namespace AnotherECS.Mathematics
{
    ...
    public partial struct float3 : System.IEquatable<float3>, IFormattable
    {
        ...
        => new float3(sfloat.Abs(v.x), sfloat.Abs(v.y), sfloat.Abs(v.z));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float3 max(float3 v0, float3 v1)
            => new float3(sfloat.Max(v0.x, v1.x), sfloat.Max(v0.y, v1.y), sfloat.Max(v0.z, v1.z));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static sfloat lengthsq(float3 v)
            => v.x * v.x + v.y * v.y;
