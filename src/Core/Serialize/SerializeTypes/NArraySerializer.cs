        ...
        where T : unmanaged
    {
        ...
        public void UnpackBlittable(ref ReaderContextSerializer reader, ref NArray<TAllocator, T> data)
        {
            ...
            if (elementCount != uint.MaxValue)
            {
                ...
                data = new NArray<TAllocator, T>(reader.Dependency.DirectGet<WPtr<TAllocator>>(allocatorId).Value, ref memoryHandle, elementCount);
                return;
            }
            data = default;
        }
        ...
        where T : unmanaged, ISerialize
    {
        ...
        public static NArray<TAllocator, T> Unpack(ref ReaderContextSerializer reader)
        {
            ...
            if (elementCount != uint.MaxValue)
            {
                ...
                var nArray = new NArray<TAllocator, T>(reader.Dependency.DirectGet<WPtr<TAllocator>>(allocatorId).Value, ref memoryHandle, elementCount);
