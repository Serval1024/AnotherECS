...
{
    public unsafe struct NDictionary<TAllocator, TKey, TValue, THashProvider> : INDictionary<TKey, TValue>, ISerialize, IRepairMemoryHandle
        ...
        where THashProvider : struct, IHashProvider<TKey, uint>
    {
        ...
        public bool ContainsValue(TValue value)
        {
            ...
            {
                if (_entries.ReadRef(i).hashCode < EMPTY && _entries.ReadRef(i).value.Equals(value))
                {
                    return true;
                }
            }
        ...
        {
            var hashCode = _hashProvider.GetHash(ref key) & MASK;
        ...
        public bool Remove(TKey key)
        {
            ...
            ExceptionHelper.ThrowIfBroken(this);
#endif
            uint hashCode = _hashProvider.GetHash(ref key) & MASK;
        ...
        public TValue Get(uint index)
        {
            ...
            while (cIndex < _count)
            {
                if (_entries.ReadRef(cIndex).hashCode < EMPTY)
                {
                    if (index == cIndex)
                    {
            ...
            where TIterator : struct, IIterator<TValue>
        {
            ...
            while (index < _count)
            {
                if (_entries.ReadRef(index).hashCode < EMPTY)
                {
        ...
        public struct Enumerator : IEnumerator<Pair<TKey, TValue>>, IEnumerator
        {
            ...
            public bool MoveNext()
            {
                ...
                {
                    if (_data._entries.ReadRef(_index).hashCode < EMPTY)
                    {
        ...
        {
            private NDictionary<TAllocator, TKey, TValue, THashProvider> _dictionary;

            public ValueCollection(in NDictionary<TAllocator, TKey, TValue, THashProvider> dictionary)
            {
            ...
            public struct Enumerator : IEnumerator<TValue>, IEnumerator
            {
                ...
                internal Enumerator(in NDictionary<TAllocator, TKey, TValue, THashProvider> data)
                {
                    _data = data;
                    _index = 0;
                    _current = default;
