...
namespace AnotherECS.Physics
{
    ...
    internal unsafe struct BlobAssetReferenceData {
        ...
        public BlobAssetReferenceData Copy()
        {
            var fullLength = sizeof(BlobAssetHeader) + Header->Length;
            byte* buffer =
                 (byte*)UnsafeUtility.Malloc(fullLength, 16, Header->Allocator);
            UnsafeUtility.MemCpy(buffer, Header, fullLength);

            return new BlobAssetReferenceData() { m_Ptr = buffer };
        }

        public int GetByteLength()
        {
            ValidateNotNull();
            return sizeof(BlobAssetHeader) + Header->Length;
        }

        public void CopyToMemory(void* ptr, uint length)
        {
            ValidateNotNull();
            var fullLength = sizeof(BlobAssetHeader) + Header->Length;
            ...
            UnsafeUtility.MemCpy(ptr, Header, fullLength);
        }
    ...
    public unsafe struct BlobAssetReference<T> : IDisposable, IEquatable<BlobAssetReference<T>>
        ...
        public BlobAssetReference<T> Copy() {
            return Create(m_data.Copy());
        }
